...
pub async fn build_step_6_final_check(
    ...
    let mut repaired_segments = request.segments.clone();
    let mut service_response =
        run_step_6_final_check_request(&request.target_lang, &repaired_segments)?;

    if !service_response.passed {
        let mut has_llm = true;
        let mut api_key = request.translate_api_key.clone();
        let mut base_url = request.translate_base_url.clone();
        let mut model = request.translate_model.clone();
        let llm_concurrency = request.llm_concurrency.max(1) as usize;
        if hydrate_translate_llm_connection_settings(&mut api_key, &mut base_url, &mut model)
            .is_err()
        {
            has_llm = false;
        }
        ...
        if has_llm {
            let hard_issue_indexes =
                collect_step6_hard_issue_indexes(&service_response.issues, &repaired_segments);
            let issue_map = collect_step6_issue_messages_by_segment(
                &service_response.issues,
                repaired_segments.len(),
                &repaired_segments,
            );
            if let Ok(repair_results) = run_step6_quality_repairs(
                &request.task_id,
            ...
            .await
            {
                let mut current_counts = count_step6_hard_issues_by_segment(
                    &service_response.issues,
                    repaired_segments.len(),
                    &repaired_segments,
                );
                for repair in repair_results {
                    let candidate = repair.translation;
                    if candidate.is_empty() {
                        continue;
                    }
                    let mut simulated_segments = repaired_segments.clone();
                    simulated_segments[repair.segment_index].translation = candidate;
                    let simulated =
                        run_step_6_final_check_request(&request.target_lang, &simulated_segments)?;
                    if simulated.hard_fail_count > service_response.hard_fail_count {
                        continue;
                    }
                    let simulated_counts = count_step6_hard_issues_by_segment(
                        &simulated.issues,
                        repaired_segments.len(),
                        &simulated_segments,
                    );
                    if simulated_counts[repair.segment_index]
                        >= current_counts[repair.segment_index]
                    {
                        continue;
                    }
                    current_counts = simulated_counts;
                    repaired_segments = simulated_segments;
                    service_response = simulated;
                }
            }
        }
    }

    let output_segments = repaired_segments
        .into_iter()
        .map(|segment| BuildTranslationSegmentCommand {
            segment_id: segment.segment_id,
            start: segment.start,
...
#[derive(Debug, Clone)]
struct Step6RepairSuggestion {
    segment_index: usize,
    translation: String,
}

fn collect_step6_hard_issue_indexes(
    issues: &[crate::services::subtitle_step5::Step5QualityIssue],
    segments: &[BuildTranslationSegmentCommand],
) -> Vec<usize> {
    let segment_id_to_index = build_segment_id_to_index(segments);
    let mut out = Vec::<usize>::new();
    for issue in issues {
        if issue.severity != "hard" {
            continue;
        }
        let Some(index) = step6_issue_segment_index(issue, &segment_id_to_index, segments.len())
        else {
            continue;
        };
...
fn collect_step6_issue_messages_by_segment(
    issues: &[crate::services::subtitle_step5::Step5QualityIssue],
    segment_count: usize,
    segments: &[BuildTranslationSegmentCommand],
) -> Vec<Vec<String>> {
    let mut out = vec![Vec::<String>::new(); segment_count];
    let segment_id_to_index = build_segment_id_to_index(segments);
    for issue in issues {
        if issue.severity != "hard" {
            continue;
        }
        if let Some(index) = step6_issue_segment_index(issue, &segment_id_to_index, segment_count) {
            out[index].push(format!("{}: {}", issue.rule_id, issue.message));
        }
    }
    out
}

fn count_step6_hard_issues_by_segment(
    issues: &[crate::services::subtitle_step5::Step5QualityIssue],
    segment_count: usize,
    segments: &[BuildTranslationSegmentCommand],
) -> Vec<usize> {
    ...
    for issue in issues {
        ...
        let Some(index) = step6_issue_segment_index(issue, &segment_id_to_index, segment_count)
        else {
            continue;
        };
...
fn build_segment_id_to_index(
    segments: &[BuildTranslationSegmentCommand],
) -> std::collections::HashMap<usize, usize> {
    let mut map = std::collections::HashMap::new();
    for (index, segment) in segments.iter().enumerate() {
        map.insert(segment.segment_id, index);
    }
    map
}

fn step6_issue_segment_index(
    issue: &crate::services::subtitle_step5::Step5QualityIssue,
    segment_id_to_index: &std::collections::HashMap<usize, usize>,
    segment_count: usize,
) -> Option<usize> {
    if issue.part_id > 0 {
        let part_index = issue.part_id.saturating_sub(1);
        if part_index < segment_count {
            return Some(part_index);
        }
    }
    segment_id_to_index.get(&issue.segment_id).copied()
}

async fn run_step6_quality_repairs(
    task_id: &str,
    media_path: String,
    source_lang: &str,
    target_lang: &str,
    ...
    issue_map: &[Vec<String>],
    api_key: &str,
    base_url: &str,
    model: &str,
    concurrency: usize,
) -> Result<Vec<Step6RepairSuggestion>, String> {
    ...
    let mut tasks = Vec::<crate::services::llm::port::LlmJsonTask>::new();
    for &segment_index in target_indexes {
        let Some(segment) = segments.get(segment_index) else {
            continue;
        };
        let Some(issues) = issue_map.get(segment_index) else {
            continue;
        };
        if issues.is_empty() {
            continue;
        }
        tasks.push(crate::services::llm::port::LlmJsonTask {
            id: segment_index,
            request_id: next_llm_request_id(),
            user_prompt: build_step6_qa_prompt(
                source_lang,
                target_lang,
                &segment.source,
                &segment.translation,
    ...
    let context = LlmCallContext {
        ...
        phase: "step_6_final_check".to_string(),
    };
                ...
                async move {
                    let call = client
                        .call_json_validated(
                            ...
                            |value| parse_step6_qa_translation(value),
                        )
                        .await
                        .map_err(|err| {
                            format!(
                                "step6 qa failed (llmId={}): {}",
                                task.request_id, err.message
                            )
                        })?;
                    Ok((task.id, call.value))
                }
            }
        },
        |msg| msg,
    ...
    let mut out = Vec::<Step6RepairSuggestion>::new();
    for (_task_id, result) in results {
        let Ok((id, text)) = result else {
            continue;
        };
        let segment_index = id;
        if target_indexes.iter().all(|index| *index != segment_index) {
            continue;
        }
        ...
        out.push(Step6RepairSuggestion {
            segment_index,
...
fn build_step6_qa_prompt(
    ...
    serde_json::json!({
        ...
        "constraints": [
            "Keep one target subtitle line.",
            "Keep meaning faithful to source text.",
            "Do not add explanations.",
            "Do not add extra fields.",
            "If uncertain, keep the original translation."
        ],
        "output": {
