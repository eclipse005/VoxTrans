...
fn extract_audio_with_ffmpeg(input_path: &Path, output_wav: &Path) -> Result<(), String> {
    ...
    let output = command
        ...
        .arg(output_wav)
        .output()
        .map_err(|err| format!("Failed to extract audio: ffmpeg call failed: {err}"))?;
    ...
    if stderr.is_empty() {
        return Err("Failed to extract audio: ffmpeg execution failed".to_string());
    }
    Err(format!("Failed to extract audio: {stderr}"))
}
