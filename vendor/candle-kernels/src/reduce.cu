...
template <typename T>
__device__ __forceinline__ void atomic_add_compat(T *address, T val) {
  atomicAdd(address, val);
}

__device__ __forceinline__ void atomic_add_compat(__half *address, __half val) {
  ...
  do {
    ...
    if ((size_t)address & 2) {
      __half new_half = __hadd(__ushort_as_half((unsigned short)(old >> 16)), val);
      new_bits = (old & 0x0000ffff) | ((unsigned int)__half_as_ushort(new_half) << 16);
    } else {
      __half new_half = __hadd(__ushort_as_half((unsigned short)(old & 0x0000ffff)), val);
      new_bits = (old & 0xffff0000) | (unsigned int)__half_as_ushort(new_half);
    }
    old = atomicCAS(address_as_ui, assumed, new_bits);
  } while (assumed != old);
#endif
}
  ...
  extern "C" __global__ void FN_NAME(                                          \
    ...
    if (is_contiguous(num_dims, dims, strides)) {                              \
      for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel;  \
        ...
        atomic_add_compat(out + dst_index, inp[i]);                            \
      }                                                                        \
    } else {                                                                   \
      for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < numel;  \
           i += blockDim.x * gridDim.x) {                                      \
        ...
        atomic_add_compat(out + dst_index, inp[strided_i]);                    \
      }                                                                        \
    }                                                                          \
  }
